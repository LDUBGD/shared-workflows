...
[ ] 3.2. Створення середовища development:
...
[x] 5.1. Фінальна перевірка: Переконатися, що у відповідних репозиторіях на GitHub створено середовища та секрети DEPLOY_PROJECT_DIR, а локально згенеровано нові файли main.yml.

[x] 5.2. Коміт та пуш: Відкрити термінал, зайти в кожну опрацьовану директорію зі списку та виконати наступні команди:
...
Ідентифікація проєктів: Для виконання GitHub API запитів щодо конкретного репозиторію, надійним джерелом ідентифікації пари owner/repo є розбір результату команди git remote get-url origin.

Пріоритет секретів: Категорично заборонено дублювати SSH ключі в Environments. Інфраструктура покладається виключно на Organization Secrets. В Environments додається ТІЛЬКИ унікальний шлях DEPLOY_PROJECT_DIR.